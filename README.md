...
- `npm run test:e2e:visual:update` обновляет screenshot baselines осознанно после UI-изменений
...
- mobile editor add/save и row duplication
- production-mode smoke для скрытого demo route и пустого login без demo-prefill
- visual regression checkpoints для `login`, `demo`, `timesheets`, `timesheet editor`
