...
* It is required to render the `Notifications` component inside [MantineProvider](https://alpha.mantine.dev/llms/theming-mantine-provider.md)
...
* `color, icon, title, radius, className, style, loading` – props passed down to the [Notification](https://alpha.mantine.dev/llms/core-notification.md) component
...
function Demo() {
  return (
    <Notification loading={false} withCloseButton={true} withBorder={false} color="blue" radius="sm" title="We notify you that" children="You are now obligated to give a star to Mantine project on GitHub">
      {{children}}
    </Notification>
  );
}
...
You can use `style`, `className` or [Styles API](https://alpha.mantine.dev/llms/styles-styles-api.md) `classNames`, `styles` props to customize notification styles.
Usually, it is better to override [Notification](https://alpha.mantine.dev/llms/core-notification.md) styles with `classNames` prop in the [theme object](https://alpha.mantine.dev/llms/theming-theme-object.md).
